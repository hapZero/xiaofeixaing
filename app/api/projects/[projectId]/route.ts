...
import { assets, audioPresets, characters, episodes, projects, shots } from "../../../../db/schema";
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const projectAudioPresets = await db.select().from(audioPresets).where(eq(audioPresets.projectId, projectId));
  const episodeShots = projectEpisodes.length
    ...
    : [];
  return json({ project, episodes: projectEpisodes, assets: projectAssets, characters: projectCharacters, audioPresets: projectAudioPresets, shots: episodeShots });
}
