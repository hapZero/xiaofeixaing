...
export async function GET(request: Request) {
  ...
  try {
    ...
    return json({ configured: true, ...result, serverUrl: getComfyUiServerUrl(), message: "Spark 连接正常" });
  } catch (error) {
    const reason = error instanceof Error ? error.message : "COMFYUI_CONNECTION_FAILED";
    return json({ configured: true, connected: false, serverUrl: getComfyUiServerUrl(), message: "无法连接 Spark", reason });
  }
}
