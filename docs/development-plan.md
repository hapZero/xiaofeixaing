...
- 已完成：工作引擎配置中心、节点输入/输出映射、Spark 连接检测和能力绑定持久化；手工上传 API JSON 仅保留为兼容模式。
- 已完成：Spark 安装小飞象桥接器、ComfyUI 工作流自动同步、API 执行版与编辑版版本固定，以及小飞象直接选择已同步工作流。
- 已完成：ComfyUI 原生节点事件采集、加权单调进度、节点内 value/max、执行事件持久化和页面实时展示。
- 已完成：生成任务自动轮询、ComfyUI 历史结果解析、生成文件归档，以及分镜首帧回填和预览。
- 已完成：已绑定工作流的独立测试运行、首帧上传、4.84 秒视频有效性验证和视频预览，以及分镜编辑器图生视频提交和结果回填。
